...
export function MusicToggle({ isPlaying, onToggle }: MusicToggleProps) {
  ...
  return (
    <motion.button
      ...
      title={label}
      className="card fixed right-4 top-4 z-50 flex h-11 w-11 items-center justify-center rounded-full text-primary tap-transparent"
    ...
    >
      {isPlaying ? (
        <Volume2 className="h-5 w-5 text-accent" aria-hidden />
      ) : (
